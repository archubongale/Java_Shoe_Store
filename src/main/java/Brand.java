...
public class Brand {
  ...
  public List<Store> getStores() {
    try(Connection con = DB.sql2o.open()) {
      String sql = "SELECT store.* FROM brands JOIN stores_brands ON (brands.id = brands_stores.brand_id) JOIN stores ON (brands_stores.store_id = stores.id) WHERE brands.id =:id ORDER BY name";
      return con.createQuery(sql)
      .addParameter("id",id)
